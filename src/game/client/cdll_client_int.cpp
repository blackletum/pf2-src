...
int CHLClient::Init( CreateInterfaceFn appSystemFactory, CreateInterfaceFn physicsFactory, CGlobalVarsBase *pGlobals )
{
	...
	// Idiot proof - Thanks Open Fortress
	if (Q_strcmp( COM_GetModDirectory(), "pf2" ) && Q_strcmp( COM_GetModDirectory(), "pf2beta" ))
	{
		ConColorMsg( Color( 60, 238, 60, 255 ), "%s\n", COM_GetModDirectory() );
		Error( "Error! The game's directory must have the exact name \"pf2\" in order for the mod to work correctly. Please change it." );
	}
